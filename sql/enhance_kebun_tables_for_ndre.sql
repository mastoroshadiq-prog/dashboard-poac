-- ================================================================
-- ENHANCE KEBUN TABLES FOR NDRE DRONE DATA
-- ================================================================
-- Date: November 12, 2025
-- Purpose: Enhance kebun_n_pokok and kebun_observasi for NDRE import
-- Impact: Support 1000+ drone survey observations
-- ================================================================
...
-- ================================================================
-- PART 2: ENHANCE/CREATE kebun_observasi (Observation Data)
-- ================================================================
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '================================================================';
  RAISE NOTICE 'ENHANCING kebun_observasi - Adding NDRE Survey Support';
  RAISE NOTICE '================================================================';
END $$;

-- 2.1 Check if table exists, if not create it
CREATE TABLE IF NOT EXISTS kebun_observasi (
  id_observasi UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  id_npokok UUID REFERENCES kebun_n_pokok(id_npokok) ON DELETE CASCADE,
...
-- 2.2 Add NDRE-specific columns (if not exists)
ALTER TABLE kebun_observasi
ADD COLUMN IF NOT EXISTS ndre_value DECIMAL(10, 8),
ADD COLUMN IF NOT EXISTS ndre_classification VARCHAR(50),
...
CREATE INDEX IF NOT EXISTS idx_kebun_observasi_npokok 
ON kebun_observasi(id_npokok);

CREATE INDEX IF NOT EXISTS idx_kebun_observasi_survey_date 
ON kebun_observasi(tanggal_survey DESC);

CREATE INDEX IF NOT EXISTS idx_kebun_observasi_jenis 
ON kebun_observasi(jenis_survey);

CREATE INDEX IF NOT EXISTS idx_kebun_observasi_ndre_value 
ON kebun_observasi(ndre_value) 
WHERE ndre_value IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_kebun_observasi_classification 
ON kebun_observasi(ndre_classification) 
WHERE ndre_classification IS NOT NULL;

-- 2.5 Create composite index for common queries
CREATE INDEX IF NOT EXISTS idx_kebun_observasi_survey_lookup 
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_kebun_observasi_unique_survey
...
DROP TRIGGER IF EXISTS trigger_kebun_observasi_updated_at ON kebun_observasi;
...
DO $$
BEGIN
  RAISE NOTICE '✅ kebun_observasi enhanced successfully!';
  RAISE NOTICE '   - Added: ndre_value, ndre_classification, metadata_json, keterangan';
...
BEGIN
  ...
  WHERE table_name = 'kebun_observasi'
    AND column_name IN ('ndre_value', 'ndre_classification', 'metadata_json', 'keterangan');
  ...
  RAISE NOTICE '  - New columns added: % / 4', v_observasi_columns;
  RAISE NOTICE '  - Indexes created: %', v_observasi_indexes;
  RAISE NOTICE '';
  
  IF v_npokok_columns = 4 AND v_observasi_columns = 4 THEN
...
FROM information_schema.columns
WHERE table_name = 'kebun_n_pokok'
  AND column_name IN ('id_npokok', 'object_id', 'divisi', 'blok', 'blok_detail', 'n_baris', 'n_pokok', 'tgl_tanam')
ORDER BY ordinal_position;

-- Show enhanced kebun_observasi structure
SELECT 
  column_name, 
  data_type, 
  is_nullable,
...
-- End of enhance_kebun_tables_for_ndre.sql
