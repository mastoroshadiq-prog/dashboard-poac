...
async function createSpkHeader(spkData) {
  try {
    console.log('📝 [Service] Creating SPK Header...');
    ...
    console.log('✅ ID Asisten valid:', asistenExists.id_pihak);
    ...
    const newSpk = data[0];
    console.log('✅ [Service] SPK Header created successfully!');
    ...
    console.error('❌ [Service] Error in createSpkHeader:', err.message);
...
async function addTugasKeSpk(id_spk, arrayTugas) {
  try {
    ...
    console.log('🔍 Tipe Tugas:', uniqueTipeTugas);
    ...
    if (tipeTugasError) {
      console.log('⚠️  Warning: Tabel tupoksi_tipe tidak ada, skip validasi tipe_tugas');
    } else {
      ...
      console.log('✅ Semua Tipe Tugas valid');
    }
    */
    ...
    console.log('📦 Prepared data untuk insert:', tugasToInsert.length, 'tugas');
    ...
    if (error) {
      console.error('❌ [Service] Supabase BATCH INSERT error:', error);
    ...
    console.error('❌ [Service] Error in addTugasKeSpk:', err.message);
    throw err; // Re-throw untuk di-catch oleh router layer
  }
}
